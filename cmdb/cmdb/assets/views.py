...
def asset_report(request):
    ...
    if request.method == 'POST':
        ...
        if ass_handler.data_is_valid(): #判断数据是否合法
            ass_handler.data_inject()   #验证通过写入数据
            #return HttpResponse(json.dumps(ass_handler.response))
        return HttpResponse(json.dumps(ass_handler.response))   #返回信息
...
@csrf_exempt
def asset_with_no_asset_id(request):    #没有资产id
    if request.method == 'POST':
        ass_handler = core.Asset(request)
...
def new_assets_approval(request):   #新资产
    if request.method == 'POST':
