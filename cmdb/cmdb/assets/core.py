...
class Asset(object):
    def __init__(self,request):
        self.request = request  #用户的请求
        self.mandatory_fields = ['sn','asset_id','asset_type'] #验证时要强制检测的字段
        self.response = {
            'error':[],
            'info':[],
            'warning':[]
    ...
    def mandatory_check(self,data,only_check_sn=False):
        for field in self.mandatory_fields: #循环将错误加入错误列表
            if not data.has_key(field): #判断客户端发送数据是否有相应的key
                self.response_msg('error','MandatoryCheckFailed', "The field [%s] is mandatory and not provided in your reporting data" % field)
        else:
            if self.response['error']:return False #有错误信息
        try:
            if not only_check_sn:  #检测sn号和id
                self.asset_obj = models.Asset.objects.get(id=int(data['asset_id']),sn=data['sn'])   #获取资产id
            else:
                self.asset_obj = models.Asset.objects.get(sn=data['sn'])
            return True
        except ObjectDoesNotExist,e: #取不到资产id
            ...
            self.waiting_approval = True    #待审批判断需要这个字段
            return False
            ...
            try:
                ...
                if self.mandatory_check(data,only_check_sn=True): #the asset is already exist in DB,just return it's asset id to client
                    ...
                    if hasattr(self,'waiting_approval'):
                        ...
                        self.save_new_asset_to_approval_zone() #将数据放到审批区
                        print response
                    else:
    ...
    def data_is_valid(self):
        ...
        if data:    #是否存在数据
            try:
                data = json.loads(data) #加载成json字符串格式
                self.mandatory_check(data)  #验证数据的合法性
                self.clean_data = data
                if not self.response['error']:  #验证数据没有有错
                    return True
            except ValueError,e:
                self.response_msg('error','AssetDataInvalid', str(e))
        else:
    ...
    def __is_new_asset(self):
        if not hasattr(self.asset_obj, self.clean_data['asset_type']):#验证时数据库取到的对象中是否是新的资产
            return True
        else:
            return False
    ...
    def data_inject(self):
        ...
        if self.__is_new_asset():   #新资产创建资产
    ...
    def __verify_field(self,data_set,field_key,data_type,required=True):
        field_val = data_set.get(field_key) #获取要验证的字段
        if field_val:   #存在
            try:
                data_set[field_key] = data_type(field_val)  #将model的值转化成字符串
            except ValueError,e:
    ...
    def _create_server(self):   #创建server表的信息
        self.__create_server_info() #创建server表信息
    ...
    def __create_server_info(self,ignore_errs=False):
        try:
            self.__verify_field(self.clean_data,'model',str)    #将model转化成str格式
            if not len(self.response['error']) or ignore_errs == True: #no processing when there's no error happend
                ...
                obj = models.Server(**data_set) #没有错误写入数据库
                obj.save()
                return obj
        except Exception,e:
    ...
    def __create_disk_component(self):
        ...
        if disk_info:
            for disk_item in disk_info: #循环硬盘字典
                try:    #确保该有数值存在
    ...
    def __update_asset_component(self,data_source,fk,update_fields,identify_field=None):
        ...
        try:
            component_obj = getattr(self.asset_obj,fk)  #通过反射获取到资产数据
            if hasattr(component_obj,'select_related'): # this component is reverse m2m relation with Asset model
                objects_from_db = component_obj.select_related()#取出数据
                for obj in objects_from_db: #让数据库数据和客户端数据对比
                    key_field_data= getattr(obj,identify_field) #找唯一标识
                    ...
                    if type(data_source) is list:
                        for source_data_item  in data_source:
                            ...
                            if key_field_data_from_source_data: #找到标识
                                if key_field_data == key_field_data_from_source_data: #找到相同的标识find the matched source data for this component,then should compare each field in this component to see if there's any changes since last update
                                   self.__compare_componet(model_obj=obj,fields_from_db=update_fields,data_source=source_data_item) #数据对比
    ...
    def __filter_add_or_deleted_components(self,model_obj_name,data_from_db,data_source,identify_field):
        ...
        if type(data_source) is list:   #取出客户端的mac
        ...
        data_identify_val_from_db = set([getattr(obj,identify_field) for obj in data_from_db])#取服务端的mac地址
        ...
        self.__delete_components(all_components=data_from_db, delete_list = data_only_in_db, identify_field=identify_field )    #删除只在数据库中有的
        if data_only_in_data_source:
            self.__add_components(model_obj_name=model_obj_name,all_components=data_source, add_list = data_only_in_data_source, identify_field=identify_field )    #增加数据库没有的数据
    ...
    def __compare_componet(self,model_obj,fields_from_db,data_source):
        ...
        for field in fields_from_db: #循环字段一一对比[ip, slot..]
            val_from_db = getattr(model_obj,field)
            val_from_data_source = data_source.get(field)
            if val_from_data_source:
                if type(val_from_db) is unicode:val_from_data_source = unicode(val_from_data_source)#转换数据类型
                ...
                if val_from_db == val_from_data_source:#资产相等 this field haven't changed since last update
                    pass
                    ...
                    db_field = model_obj._meta.get_field(field)    #通过字段获取对象，数据更新
