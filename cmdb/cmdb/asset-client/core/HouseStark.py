...
class ArgvHandler(object):
    ...
    def parse_argv(self):   #判断是否存在相应的方法然后调用
        if len(self.argvs) >1:
    ...
    def __submit_data(self,action_type,data,method):
        ...
        if action_type in settings.Params['urls']:
            if type(settings.Params['port']) is int:    #端口是整数，不是默认80端口
    ...
    def load_asset_id(self,sn=None):
        ...
        has_asset_id = False
        if os.path.isfile(asset_id_file):   #文件存在
    ...
    def report_asset(self):
        ...
        response = self.__submit_data(post_url,data,method="post")  #提交数据
        if "asset_id" in response:
            self.__update_asset_id(response["asset_id"])    #更新资产id

        self.log_record(response)   #存取日志
